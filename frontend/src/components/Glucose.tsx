...
const Glucose = () => {
    const [yValues, setYValues] = useState<string[]>(Array(6).fill(''));
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            const response = await axios.post('https://api.ikeda042api.net/api/lab/glucose', { y: yValuesNumber });
    ...
    return (
        <Container>
            <DrawerAppBar />
            <Box mt={12} mb={20}>
                <Typography variant="h5" align='center' gutterBottom>Glucose 検量線データの入力</Typography>
                <form onSubmit={handleSubmit}>
                    <Table>
                        <TableHead>
                            <TableRow>
                                <TableCell align="center" style={{ width: '50%' }}>Glucose濃度(g/L)</TableCell>
                                <TableCell align="center" style={{ width: '50%' }}>ΔOD340(-)</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
